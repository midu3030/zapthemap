...
mails = zeros((db.alltextmail.count(), 2000)) 
mongomails = db.alltextmail.find({}, {"wordfreqs" : 1})
mongomails.batch_size(10000)

i = 0
for elem in mongomails: 
    freqs = cPickle.loads(elem['wordfreqs'])
    for attrib in xrange(2000):
        mails[i][attrib] = freqs[attrib]
    i = i + 1

result = calc_tsne(mails)

print("ready")
#vectors = array([elem for elem in db.alltextmail.find()])
